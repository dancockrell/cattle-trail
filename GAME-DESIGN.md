...
After Ada accepts the outfit invitation, a real wooden/brass two-seat cart appears on the lower trail. Board to control Ada in the occupied cart, inspect its regulator, open outer valves A/C and leave middle bypass B closed. A wrong test produces one point of Ada strain and permits a retry. Follow three physical brass-lantern checkpoints in order, return to the cart stop and finish for15trust once. An optional kiss is a separate mutual choice, remembered once and worth5additionaltrust. Romance is not required to drive or finish.

The room transfers movement to the whole occupied-cart actor; the trail-boss and standingAda sprites hide during the outing. Pausing parks an empty cart, returns playercontrol and preserves the actual drivingposition/checkpoints for resumption. After theouting Ada stands beside the cart for the closing exchange. The source save includes allactivityflags and cartposition. Old saves without this activity remain supported.

Static cart artwork provides occupiedSE/S/SW/N plus emptyparkedSE views. E/W inputs use their closest quarter-view, and northern diagonals useN. These mappings are explicit in the sprite metadata; wheel cycles and interpolatedcartturns are not authored. The incorrect original rearboiler pose was excluded and replaced with a completewoodenrear view. The outing passed backend, controller and save-state checks without a new renderedgame run orWindows export.

Cart steering now accelerates toward 32 world pixels/second at48pixels/second squared, brakes at64, and turns at a maximum2.5radians/second. These are provisional play values. Reverse input steers through a turn; releasing directional input brakes. Brake[E] clears a click destination and stops the cart immediately during the lantern route. The cart uses15pixels of extra scenery/wagon/world-edge clearance; human and cattle footprints retain their existing size. Heading is saved; velocity is stopped on pause, boarding, completion and load.
