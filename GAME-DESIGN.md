...
- **Ada — Two Seats, One Regulator (built):** play as Ada, route pressure through a stalled two-seat steam cart while the protagonist braces its jumpy controls; take a short successful ride together. Her delighted teasing and an optional kiss at the stop are the payoff. Use a compact readable repair challenge, not a long engineering menu.
- **Ines — The Moon's Wrong Reflection (not built):** play as Ines, follow moving spirit reflections across a safe stretch of creek while the protagonist follows her signals. Find the real crossing, then enjoy a playful secluded moment and an optional kiss. Her special perception is used directly by the player rather than described in a dialogue tree. This adventure sketch predates and is carried over from the retired Rosa Vale draft; it is not yet built. Ines's own current room encounter (the three-clue spirit trail, recruitment and camp rest) is built and is a separate, already-shipped thing from this sketch.

Eleanor's and Ada's sketches above are implemented and playable now (verified 2026-09-09 with `python tools/verify_godot.py lantern lantern-controller lantern-view ada ada-cart ada-cart-controller mechanic-controller`); only Ines's sketch remains design only. Keep the relationship experience varied through different skills and situations; procedural companions draw compatible adventure templates instead of recycling the same conversation with a different name.
...
Ada Mercer now has a source-room steam repair encounter, recruitment, cart outing and camp care. Her richer wardrobe studies remain separate candidates. Ines Vale (23) has a persisted spirit-trail encounter, recruitment, camp/field assignment and shared camp rest; see her full record below and in design/characters.json. She supersedes the earlier Rosa Vale draft of the same spirit-listener/scout role, which no longer exists as a separate character.
...
Ada first-encounter backend: the stalled agricultural walker now has a fictional pressure puzzle. Shut feed, vent to 5 or below, install the recovered regulator, close vent, build pressure into 30–50, close feed and test. Overpressure above 85 faults the machine and shuts feed; venting allows retry without losing the part. Successful testing locks the repaired state. Ada's adult, 22, identity, madness, first meeting, trust and invitation are separate persisted state. Repair must precede an accepted invitation, which grants trust once without romance. Outer saves retain this optional state and accept older saves without it. The source room now places Ada and the walker after the lantern crossing and activates contextual Feed, Vent, Install/Test and Invite controls. This agricultural walker encounter remains distinct from the later Two Seats, One Regulator romance adventure.


## Persistent generated companions and capped perks

The source now stores resolved generated companion identities inside the room save, without rerolling names or appearances on load. Recruitment, relationship choice, camp/field assignment and one-time activity rewards are separate. Madness retains fractional values. The roster has no newly imposed population limit; adult identity, unique visual IDs and finite values are validated atomically. Production-preview artwork cannot enter the default live roster. The 96 character appearances remain an offline candidate library until their encounter and art admission are authored.

The perk resolver uses authored values and activation rules instead of trusting numbers in a save. Eleanor's existing Steady Company recovery now uses it and still gives 3 additional player recovery. Recruited rancher-family helpers add 2 seconds to a successful cattle lasso's following duration, capped at 4 seconds across the outfit (18 seconds becomes20 or22). This hook is implemented; the current room has no admitted generated rancher encounter yet. Mechanic repair efficiency and spirit-scout warning tiers have capped resolver definitions but no new consuming gameplay action, so those effects remain backend preparation. Recruitment never starts romance or increases an effect according to clothing.


## Two Seats, One Regulator source outing

After Ada accepts the outfit invitation, a real wooden/brass two-seat cart appears on the lower trail. Board to control Ada in the occupied cart, inspect its regulator, open outer valves A/C and leave middle bypass B closed. A wrong test produces one point of Ada strain and permits a retry. Follow three physical brass-lantern checkpoints in order, return to the cart stop and finish for 15 trust once. An optional kiss is a separate mutual choice, remembered once and worth 5 additional trust. Romance is not required to drive or finish.

The room transfers movement to the whole occupied-cart actor; the trail-boss and standing Ada sprites hide during the outing. Pausing parks an empty cart, returns player control and preserves the actual driving position/checkpoints for resumption. After the outing Ada stands beside the cart for the closing exchange. The source save includes all activity flags and cart position. Old saves without this activity remain supported.

Static cart artwork provides occupied SE/S/SW/N plus empty parked SE views. E/W inputs use their closest quarter-view, and northern diagonals use N. These mappings are explicit in the sprite metadata; wheel cycles and interpolated cart turns are not authored. The incorrect original rear boiler pose was excluded and replaced with a complete wooden rear view. The outing passed backend, controller and save-state checks without a new rendered game run or Windows export.

Cart steering now accelerates toward 32 world pixels/second at 48 pixels/second squared, brakes at 64, and turns at a maximum 2.5 radians/second. These are provisional play values. Reverse input steers through a turn; releasing directional input brakes. Brake [E] clears a click destination and stops the cart immediately during the lantern route. The cart uses 15 pixels of extra scenery/wagon/world-edge clearance; human and cattle footprints retain their existing size. Heading is saved; velocity is stopped on pause, boarding, completion and load.
...
Recruited Ines now offers the same 30-minute, up-to-10-each shared rest while assigned to camp and within 45 units of her actor. Her participant deadline shares the existing player cooldown with Eleanor and Ada. Field assignment, active outings and unfinished actions prevent Ines rest. Romance is not required; a short spirit-themed remark accompanies the quiet time. Her nearby madness display and Rest control are connected without duplicate HUD entries.

The stationary mechanic encounter now uses Ada's detailed elevated 160-pixel master at four source pixels per world unit, preserving the same approximate world height as her older coarse sprite. The steam handler's existing state art is unchanged. This selects one actual idle figure for the stationary NPC; it does not admit uncorrected walking or turning sheets.
...
Ines Vale (23) now has a persisted encounter model, distinct from the earlier Rosa draft. Meet her, investigate bell tracks, cold ashes and a wrong shadow, then complete the trail for a one-time trust reward. Accepted recruitment assigns her to camp; explicit field assignment activates her existing Spirit Sense perk record. A separate mutual romance acknowledgement never follows automatically from recruitment. Deferred choices preserve ordinary progression. Exact earned rewards, clue identities, adult identity and finite madness are validated before restoring a save. Older saves initialize her untouched state. Short action-time remarks are authored in `data/ines_banter.json`. Her source-room encounter now unlocks after Ada’s completed cart outing. The actual elevated Ines sprite stands by the northern trail; Talk reads three nearby scenery landmarks, then returns to Ines for completion and a separate invitation. Flirt remains optional. Field assignment extends landmark warnings from 24 to 36 world units, while reading still requires approach within 24; revisits grant no rewards. Contextual desktop/mobile controls and strict prerequisite-aware saves are connected. Ines is stationary during this encounter; player-controlled scouting and finished movement animation remain future work. Controller behavior is checked headlessly; final room visual acceptance and a rebuilt Windows executable remain outstanding.

Ines’s discovered trail now has three actual sprite landmarks: a spectral brass bell, cold embers and a crow shadow with no bird overhead. Their selected whole source pixels appear after meeting her, at the existing clue positions, below actors and without collision changes. Static selections are distinct from the four unfinished effects loops.
