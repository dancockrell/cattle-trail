...
def build():
    ...
    for relative, character_id, endviews in PACKAGES:
        ...
        texture_name = metadata.get('texture', metadata.get('atlas'))
        ...
        for ordinal, record in enumerate(metadata['frames']):
        ...
        spec = {
            ...
            'directional': endviews, 'default_facing': 'east',
        ...
        total += len(frames)
    print(f'DETAILED ACTOR SPECS PASS: {len(PACKAGES)} packages, {total} real whole figures, source hashes, alpha, 160px detail, pivots; zero admitted movement clips')
