...
CHECKS = {
    ...
    'location': (['--headless', '--script', 'tools/location_test.gd'], r'LOCATION PASS: \d+ checks; Clear Fork loads from data/locations/clear_fork\.json, \d+ location file\(s\) audited, and a location with a cast member outside bounds, an unresolvable id, a missing neighbour or a stranded footprint is refused by name without half loading'),
    ...
    'travel': (['--headless', '--script', 'tools/travel_test.gd'], r'TRAVEL PASS: \d+ checks'),
    'encounters': (['--headless', '--script', 'tools/encounter_table_test.gd'], r'ENCOUNTER TABLE PASS: \d+ checks, 0 failures'),
