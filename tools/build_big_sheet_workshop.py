...
<p class="details">These are actual extracted cells in source order. Playback repeats the selected row for inspection; it does not certify a seamless loop. Missing or repeated phases remain visible.</p><nav><a id="source">Original sheet</a><a id="prompt">Exact prompt</a><a id="metadata">Sprite metadata</a></nav></div>
<div><img id="full" class="sheet" alt="Full transparent sprite atlas"></div></section></main>
<script>const sheets=DATA;const $=s=>document.querySelector(s);const select=$('#sheet'),rows=$('#row'),canvas=$('canvas'),ctx=canvas.getContext('2d');let sheet,row,frame=0,playing=true,last=0;const art=new Image();let loading=false;
...
function chooseRow(){row=sheet.rows[Number(rows.value)];frame=0;last=performance.now();draw();}
...
select.onchange=choose;rows.onchange=chooseRow;$('#play').onclick=()=>{playing=!playing;$('#play').textContent=playing?'Pause':'Play';last=performance.now()};function step(delta){playing=false;$('#play').textContent='Play';frame=(frame+delta+row.frames.length)%row.frames.length;draw()}$('#prev').onclick=()=>step(-1);$('#next').onclick=()=>step(1);$('#fps').oninput=()=>$('#rate').textContent=$('#fps').value+' fps';
function tick(now){if(playing&&row&&!loading){const dt=1000/Number($('#fps').value);if(now-last>=dt){const count=Math.floor((now-last)/dt);frame=(frame+count)%row.frames.length;last+=count*dt;draw()}}requestAnimationFrame(tick)}if(sheets.length)choose();requestAnimationFrame(tick);
</script></html>'''
(BASE/'index.html').write_text(template.replace('DATA',json.dumps(records).replace('<','\\u003c')),encoding='utf-8')
