...
<script>const sheets=DATA;const loops=LOOPS;const $=s=>document.querySelector(s);const select=$('#sheet'),rows=$('#row'),canvas=$('canvas'),ctx=canvas.getContext('2d');let sheet,row,frame=0,playing=true,last=0;const art=new Image();let loading=false;
