...
func _initialize() -> void:
	...
	queue.offer("old",{},1)
	queue.offer("new",{},0)
	check(queue.offer("urgent",{},2),"Strictly higher insertion replaces lowest priority entry")
	check(queue.take().beat == "urgent" and queue.take().beat == "old","Older higher priority survives newer low priority")
	queue.offer("important",{},5)
	queue.offer("incidental",{},1)
	check(queue.offer("critical",{},6),"Critical speech accepted into full queue")
	check(queue.take().beat == "critical" and queue.take().beat == "important","Priority six evicts one rather than five")
	queue.offer("earlier",{},1)
	queue.offer("later",{},1)
	check(queue.offer("urgent_tie",{},2),"Urgent insertion into equal priority queue")
	check(queue.take().beat == "urgent_tie" and queue.take().beat == "earlier","Equal lowest priority eviction preserves earlier arrival")
