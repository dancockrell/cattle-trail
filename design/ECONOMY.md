...
This is the design for the county's economy. It is meant to be a real simulation the whole game runs on, not flavor text around a companion-collector. It exists so systems work has a coherent market to plug into instead of inventing prices and actors ad hoc, one feature at a time.

**Implemented so far:** the shrewd-dealing section below is real code. `scripts/trade_negotiation.gd` runs the accept/counter-once/walk beat, the per-buyer 0-5 reputation with its +1/neutral/-2 deltas, and the reputation-to-price nudge. Its check is `python tools/verify_godot.py trade-negotiation`, which runs `tools/trade_negotiation_test.gd`. The market pricing factors, the trade-goods table, and debt and financing are still design and have no code behind them yet.
...
**Open question:** exact base price and the numeric weight of each factor above are still to be set. The reputation factor is the one that already has a number behind it (see below); the rest do not.

## Shrewd dealing, as a mechanic

Shrewd dealing is one of the four growth levers, alongside drives, exploration, and romance. This section is implemented in `scripts/trade_negotiation.gd`; run `python tools/verify_godot.py trade-negotiation` to check it.

**1. The negotiation itself: a single counter-offer beat, not a tree.** The house style for this kind of interaction is to avoid endless conversation trees, begging, and constant refusal/persuasion loops, and a sale follows the same rule rather than inventing a separate, wordier standard for money. The buyer states an opening price (already the output of the cattle-market factors above — count/condition, venue, season/tension, reputation). The player has exactly one response: accept, counter once with a number, or walk. A counter either lands (small gain, buyer's patience/reputation tolerates it) or doesn't (buyer holds firm, or — for a rigged actor like Chester Vane — the "counter" is itself the bluff moment, since Vane's whole role in `conflicts-and-rivalries.md` is that his auctions aren't honest in the first place). No second round, no dialogue tree: one offer, one counter, one outcome. This keeps a sale a fifteen-second beat that reads as a real choice rather than a haggling minigame, and it's a small enough surface that it can plug into any buyer (Birdena Holt, Chester Vane, a rival outfit) without per-buyer scripting beyond the numbers.

**2. Reputation with a buyer.** The market section above names buyer reputation as a price factor; this is the curve, and it is live in `scripts/trade_negotiation.gd`. The numbers are first-pass values to tune against play, not finished balance:

- Reputation is per-buyer, ranges 0–5, starts at a neutral middle value of 2.
...
- Each point of reputation above the neutral 2 nudges the buyer's opening price in the player's favor by a small percentage. `REPUTATION_PRICE_PERCENT_PER_POINT` in `scripts/trade_negotiation.gd` currently sets that at 2% per point, a first-pass value to tune.
- A counter that lands raises the price by `COUNTER_GAIN_FRACTION`, currently 10%, also first-pass.

**3. Debt and the Widow Stroud.** The "Debt and financing" section below establishes that the Widow Stroud lends at real interest cost and that Magistrate Holt is where disputes escalate. This is the shape of that arc. No code implements it yet:

- A loan carries a simple flat per-period interest rate (starting point: 10% per drive-cycle, not per day, matching the game's drive-cycle pacing rather than a real-time clock), added to principal each cycle it's outstanding.
- Missing three consecutive repayment windows moves the debt from "Stroud's own ledger" to "escalated," Mortimer Vance's collecting having failed to resolve it, at which point it becomes Magistrate Holt's matter. What escalation costs (seizure, forced sale, standing loss) is still open, below.
- None of this reads as punitive dice-rolling: the player always sees the current balance and the escalation threshold before it's crossed, matching this project's existing "late signs should visibly move prices, not just flavor text" standard from the market section.

**4. A shrewd-dealing perk, on the existing perk system.** `field_repairs`/`ada_mercer_perk`, `steady_herd`, and `spirit_sense` (per "What the player invests in," reading `scripts/companion_perks.gd`) are the existing precedent: a companion's stat reduces a real cost or risk. A shrewd-dealing perk extends that same system rather than needing a new category. The effect is a small, fixed improvement to the counter-offer's odds of landing and/or a reputation-gain bonus, read from the same companion-stat mechanism the three existing perks already use. No perk id exists for this yet; `scripts/trade_negotiation.gd` currently reads no perk at all, so wiring one in is the next step here.
...
- Exact numeric prices, and the numeric weight of each cattle-market factor other than reputation. Nothing computes an opening price yet, so this is the decision that unblocks connecting `trade_negotiation.gd` to an actual sale.
- The shipped first-pass numbers to tune against play: the reputation deltas (+1/-2, cap 5), the 2% per-point price nudge, and the 10% counter gain, all in `scripts/trade_negotiation.gd`.
- What "escalated to Magistrate Holt" does mechanically (seizure, forced sale, standing loss, or something else). The trigger condition is set, the consequence is not.
- Whether the shrewd-dealing perk gets its own perk id in `companion_perks.gd` or reuses an existing one.
...
- Whether to relocate part of the trade-goods table to a third location. The "Where" column was settled 2026-09-10: `characters.json`, `npcs.json`, and the wiki organization pages named no location for any of the twenty formerly-unplaced characters (their wiki entries are trail encounters, not shop listings), so they were placed per this doc's two-market-town rule, with four exceptions where a character's own conflict names a place (Faustina Achebe's hives are on-trail at Split Oak Camp, Adelheid Kraus's still fits Bellhollow's isolation given the tax scrutiny on it, Romy Fenwick's mill sits on the water actually being diverted at The Narrow Water, Briony Wexford's dairy is a farm adjacent to Fort Griffin rather than in it). That leaves most craft goods split between Fort Griffin and Providence. If the leather-goods cluster in particular reads as too crowded once the county has more built rooms to compare against, move some of it.
