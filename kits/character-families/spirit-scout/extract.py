...
for i in range(16):
    ...
    frame.save(HERE / f'spirit_scout_{i+1:02}.png')
    atlas.paste(frame, (col * 64, row * 64))
    variants.append({'id':f'spirit_scout_{i+1:02}', 'rect':[col*64,row*64,64,64], 'age':23+(i%4), 'status':'candidate', 'pose':'idle', 'direction':'southeast', 'source_cell':list(box), 'source_trim_bounds':list(bounds), 'frame':f'kits/character-families/spirit-scout/spirit_scout_{i+1:02}.png', 'animations':{'idle':{'frames':[i],'fps':0,'loop':False}}})
atlas.save(HERE / 'atlas.png')
atlas.resize((1024,1024),Image.Resampling.NEAREST).save(HERE / 'atlas-review.png')
metadata = {'schema_version':1,'id':'spirit-scout','master_name':'Ines Vale','master_age':23,'master_image':'kits/character-families/spirit-scout/master-v1.png','variant_sheet':'kits/character-families/spirit-scout/variants-v1.png','atlas':'kits/character-families/spirit-scout/atlas.png','cell_size':[64,64],'anchor':[32,61],'variants':variants,'generation':{'tool':'builtin image_gen','date':'2026-09-08','model':'not exposed by tool','master_prompt':'master.prompt.txt','variant_prompt':'variants.prompt.txt'},'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'master_sha256':hashlib.sha256((HERE/'master-v1.png').read_bytes()).hexdigest(),'transform':'4x4 whole-cell extraction, magenta chroma key, whole-figure trim, nearest neighbor scale to40px height, baseline61; no body repaint or recombination','admission':'candidate static idle art; no walk or turn cycle; not shipped runtime','master_limitations':['Costume reference is a soft illustration with opaque gradient backdrop, not runtime art.']}
