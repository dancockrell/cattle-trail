...
The expanded library currently contains 776 extracted candidates across 13 families. Press **K** in the game or use **Open Sprite Kits.cmd** to inspect them in Godot. The default room now uses 322 selected actor frames and 30 scenery variants in 40 placements, including eight-way rider/cattle poses, corrected cardinal facing strips and the first northeast sequence repair. `assets/room-art.json` defines that selection; `ROOM-INTEGRATION.md` records its checks and remaining visual limits. Use `--original` to run the previous 24-frame comparison room.
