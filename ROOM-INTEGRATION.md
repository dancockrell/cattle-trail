...
The default room uses 330 distinct actor frames, including the recovered original wagon, and 30 scenery variants in 40 placements. The source library contains 948 extracted candidate cells across 13 families. These are separate counts; extraction is not animation acceptance. Press K for the candidate browser. Original comparison art remains available with --original.
...
The user now authorizes building out the design while animation repair continues. The full spirit-lantern encounter, travel, procedural companions and machines remain future work. Current art selection contains 330 actor frames; inventory is 948 extracted candidates, including four northern turn poses. NW to N and N to NE use authored grounded and passing bridges in both directions. The rider uses v10 northwest grounded phases, and Eleanor uses the bag-free v5 east walk. The lasso's attached lead and rear neck arc are behind bodies, with only the near neck wrap in front.

Lantern backend foundation: outer room saves now carry an optional lantern_adventure record. Older saves without it load a fresh unstarted record. Snapshot validation checks the adventure before live mutation and rejects started checkpoints without recruitment. Restoring a room also clears old speech presentation. The existing companion control flow has not yet activated this adventure; ford layout, lantern-carry locomotion and state-driven spirit interaction remain integration work.

Optional encounter art bundle: assets/lantern-art.json and assets/lantern/*.png package seven exact existing whole sprites using the standard actor schema. Eleanor has E/NE/S static lantern poses; the wrong-hand NW is excluded. The crossing spirit has four named state clips. scripts/lantern_presentation.gd derives objective, control identity, lantern visibility and spirit state from authoritative checkpoints, while scripts/lantern_view.gd renders a supplied spirit specification without changing gameplay. These modules and the export inclusion are prepared for the encounter controller; the current room does not instantiate them or start the lantern adventure yet.
...
East carry follow-up: during the lantern activity, Eleanor now uses the whole-character east carry bank from assets/lantern/eleanor_carry_east.json. That bank has four ordered walk poses and a separateidle; it replaces her original visible sprite only while facing east. Other directions retain the original character with the belt lantern. Entering the carry bank preserves elapsed gait phase. Source-only verification remains the current boundary.
